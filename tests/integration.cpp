...
namespace {
// A node on loopback and an arbitrary port, with nothing mounted that would go
// looking for peers: the announced set is then exactly the one socket it bound.
...
// Starts such a node and answers the set it announces. The assertion macros
// throw, so a failed start is reported as itself rather than as a puzzling
// address list further down.
std::vector<std::string> announcedAddrs(AnnouncedAddressPolicy policy) {
    Libp2pModuleImpl node(loopbackNode(policy));
    LOGOS_ASSERT_TRUE(node.start().success);
    auto addrs = getPeerInfoPair(node).second;
    (void)node.stop();
    return addrs;
}
}
...
LOGOS_TEST(integration_announced_address_policy_reaches_the_switch) {
    ...
    LOGOS_ASSERT_NE(unfiltered[0], std::string("/ip4/127.0.0.1/tcp/0"));
